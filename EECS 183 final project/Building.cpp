...
void Building::spawnPerson(Person newPerson){
    ...
    int floorNumber = newPerson.getCurrentFloor();
    floors[floorNumber].addPerson(newPerson, newPerson.getTargetFloor() - newPerson.getCurrentFloor());
}
...
void Building::update(Move move){
    ...
    {
        int floor = elevators[move.getElevatorId()].getCurrentFloor();
        int array[MAX_PEOPLE_PER_FLOOR];
        move.copyListOfPeopleToPickup(array);
        floors[floor].removePeople(array, move.getNumPeopleToPickup());
        elevators[move.getElevatorId()].serviceRequest(move.getTargetFloor());
    }
    else if(move.isPassMove() == false)
    {
        elevators[move.getElevatorId()].serviceRequest(move.getTargetFloor());
    }
    else
...
int Building::tick(Move move){
    ...
    {
        total = total + floors[i].tick(time);
